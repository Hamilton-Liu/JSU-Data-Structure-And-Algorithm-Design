...
public:
	...
	//3-1
	Node<DataType>* LocateAddress(const int i);				//查找地址
	//3-2
	int Frequency(const DataType &e);	//统计等于e的元素个数
};
...
Node<DataType>* LinkList<DataType>::LocateAddress(const int i) {
	Node<DataType> *p = head->next;   					//p指向第一个元素节点
	int count = 1;										
	while(p != NULL && count != i) {					//从第一个元素节点开始依次进行比较元素的值
		p = p->next;									//移动指针p使之指向后继节点
		count++;
	}
	return p ? p : NULL;								//找到元素p的地址，
} 

//3-2 统计函数：统计单链表中等于给定值e的元素个数。(内部方法)
template <class DataType>
int LinkList<DataType>::Frequency(const DataType &e){
