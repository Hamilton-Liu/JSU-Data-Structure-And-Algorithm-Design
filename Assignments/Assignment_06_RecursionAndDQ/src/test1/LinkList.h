...
protected:
	...
	int length;											//链表长度
	void Reverse(Node<DataType> *&h, Node<DataType> *&f, Node<DataType> * &newhead);					//逆置的内部辅助函数
public:
	LinkList();											//构造函数，构造空链表
...
LinkList<DataType>::LinkList(DataType a[], int n) {  	//构造函数，创建一个非空链表
	...
	p = head = new Node<DataType>;    					//创建第一个节点
	for(int i = 0; i < n; i++) {
...
Status LinkList<DataType>::GetElem(int i, DataType &e) {	//取元素
	...
	for(count = 1; count < i-1; count++)   				//后移p使之指向第i个元素节点
		p = p->next;
...
Status LinkList<DataType>::InsertElem(int i, DataType &e) {	//插入元素
	...
	for(count = 1; count < i-1; count++)				//移动指针p，使得p指向待插位置的前驱
		p = p->next;
	...
	if(i == 1){
		q = new Node<DataType>(e,head);					//创建一个新节点，next指向原来的第i个元素节点
		head = q;
	}

	else{
		q = new Node<DataType>(e, p->next);
...
Status LinkList<DataType>::DeleteElem(int i, DataType &e) {	//删除第i个元素，通过e反馈给主调函数
	...
	Node<DataType> *p = head, *q;						//p初始指向头节点，即第1个元素节点的前驱
	int count;											//计数器，记录p指向第几个元素节点的前驱
	for(count = 1; count < i-1; count++)	 			//移动指针p，使其指向待删节点的前驱
		p = p->next;
	q = p->next;	     								//获取待删除节点
...
void LinkList<DataType>::Traverse(void(*visit)(const DataType &)) const {
	Node<DataType> *p = head;   						//p指向第一个元素节点
